...
def evaluate(ready):
    ...
    # Print MLproject parameter(s)
    logger.info(f"Here now...MLflow parameters: ready={ready}, run_id={run_id}, experiment_id={experiment_id}")
        ...
        if ready:
            ...
            if version:
                baseline_model = ray.get(controller.load_model.remote(
                    ...
                    model_uri=f'models:/baseline_model/{version}'))
                data = ray.get(
                    controller.get_dataframe_from_dict.remote(run_id=run_id, artifact_name='dataset_snapshot'))
